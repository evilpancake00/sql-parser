...
class Parser {
    ...
    private object RestrictionVisitor : SqlParserBaseVisitor<Restriction>() {
        override fun visitSearch_condition(ctx: Search_conditionContext): Restriction =
            if (ctx.search_condition_and().size == 1) visit(ctx.search_condition_and()[0]) else Or(
                ctx.search_condition_and().map(this::visit)
            )
        override fun visitSearch_condition_and(ctx: Search_condition_andContext): Restriction =
            if (ctx.search_condition_not().size == 1) visit(ctx.search_condition_not()[0]) else And(
                ctx.search_condition_not().map(this::visit)
            )
